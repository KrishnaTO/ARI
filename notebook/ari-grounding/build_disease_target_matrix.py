...
notebook/ari-grounding/target_labels.json (see resolve_target_labels.py).
"""
import csv
import json
...
i_url = names.index("Target URL")
for row in ws.iter_rows(min_row=2):
    row[i_status].fill = STATUS_FILL[row[i_status].value]
    url = row[i_url].value
    if url:
        row[i_url].hyperlink = url
        row[i_url].font = Font(color="0563C1", underline="single")
...
for prefix, db_name, _ in TARGETS:
    ...
    ws2.append([prefix, db_name, len({r[0] for r in conf}), len(conf),
                ...
                sum(1 for r in sub if r[i_status] == STATUS_UNREVIEWED)])
ws2.append([])
...
for idx, width in enumerate([18, 26, 26, 20, 20, 22, 22], start=1):
